...
from matching_data import get_evolution_exponential_weighted_decay, get_tokenization, get_complete_feedback, profiles_feature_extraction
...
def retrain(user_id: int, group_peers_ids: List[int], user_feedback: int, tokenizer, optimizer, model, matching_loss, device):
  ...
  for peer_id in tqdm(group_peers_ids):
    user_embeddings = model(get_tokenization(user_id, tokenizer).to(device))[0]  # the last hidden-state is the first element of the output tuple
